        ...
        val MIGRATION_7_8 =
                object : androidx.room.migration.Migration(7, 8) {
                    override fun migrate(db: androidx.sqlite.db.SupportSQLiteDatabase) {
                        // ... (existing code, compacted for brevity in replacement if needed, but
                        ...
                        db.execSQL("ALTER TABLE `gastos_fijos` RENAME TO `gastos_fijos_old`")
        ...
        val MIGRATION_11_12 =
                object : androidx.room.migration.Migration(11, 12) {
                    override fun migrate(db: androidx.sqlite.db.SupportSQLiteDatabase) {
                        ...
                        // SQLite doesn't support adding FK columns easily with ALTER TABLE in older
                        // versions reliably for all constraints,
                        // but simpler ADD COLUMN works for nullable fields without strict
                        // constraints immediately or if we accept no strict FK enforcement on this
                        // column for legacy data
                        // To be safe and consistent with previous migrations (re-creating table for
                        // FKs):
